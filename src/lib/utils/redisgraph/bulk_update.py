...
class BulkUpdate(abc.ABC):
    ...
    @abc.abstractmethod
    def _set_query(self, filename: str, variable_name) -> str:
        ...
...
class NodeBulkUpdate(BulkUpdate):
    def _set_query(self, filename: str, variable_name: str, separator: str) -> str:
...
class RelationBulkUpdate(BulkUpdate):
    def _set_query(self, filename: str, variable_name: str, separator: str) -> str:
        basename: str = os.path.basename(filename)
        with open(filename, "rt") as f:
            headers = next(f).split(separator)
