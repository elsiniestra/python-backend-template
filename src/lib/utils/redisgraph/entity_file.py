...
def convert_schema_type(in_type):
    try:
        ...
        if in_type.startswith("ID"):
            ...
            raise Exception("Encountered invalid field type '%s'" % in_type)
    ...
    if prop_type == Type.ID_INTEGER or prop_type == Type.LONG:
        try:
            ...
            if prop_type == Type.LONG:
                raise Exception("Could not parse '%s' as a long" % prop_val)
        ...
        try:
            ...
            if prop_type == Type.DOUBLE:
                raise Exception("Could not parse '%s' as a double" % prop_val)
        ...
        if prop_val.lower() == "false":
            ...
            raise Exception("Could not parse '%s' as a boolean" % prop_val)
        ...
        if prop_val[0] != "[" or prop_val[-1] != "]":
            raise Exception("Could not parse '%s' as an array" % prop_val)
...
class EntityFile(object):
    ...
    def convert_header_with_schema(self, header):
        ...
        for idx, field in enumerate(header):
            ...
            if len(pair) > 2:
                raise Exception("%s: Field '%s' had %d colons" % (self.infile.name, field, len(field)))
            ...
            if len(pair[0]) == 0 and col_type not in (
                ...
                raise Exception("%s: Each property in the header should be a colon-separated pair" % (self.infile.name))
            else:
