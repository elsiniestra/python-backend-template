...
class RelationType(EntityFile):
    def __init__(self, query_buffer, infile, type_str, config):
        super().__init__(infile, type_str, config)
        self.query_buffer = query_buffer

    def process_schemaless_header(self, header):
        if self.column_count < 2:
            raise Exception("Relation file '%s' should have at least 2 elements in header line." % (self.infile.name))
    ...
    def post_process_header_with_schema(self, header):
        ...
        if self.types.count(Type.START_ID) != 1:
            raise Exception("Relation file '%s' should have exactly one START_ID column." % (self.infile.name))
        if self.types.count(Type.END_ID) != 1:
            raise Exception("Relation file '%s' should have exactly one END_ID column." % (self.infile.name))
    ...
    def process_entities(self):
        ...
        for row in self.reader:
            ...
            try:
                ...
                raise Exception("%s:%d %s" % (self.infile.name, self.reader.line_num, str(e)))
            ...
            if (
                ...
                or self.query_buffer.buffer_size + added_size >= self.config.max_buffer_size
            ):
