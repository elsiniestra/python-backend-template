...
async def run(client, graphname, args):
    result = await client.execute_command("GRAPH.BULK", graphname, *args)
...
class QueryBuffer:
    ...
    async def send_buffer(self):
        ...
        args = (
            [self.node_count, self.relation_count, len(self.labels), len(self.reltypes)] + self.labels + self.reltypes
        )
    ...
    async def add_task(self, task):
        ...
        if len(self.tasks) == 5:
            ...
            stats = await task.get()
            self.update_stats(stats)

    async def wait_pool(self):
